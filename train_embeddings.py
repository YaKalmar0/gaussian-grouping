...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, use_wandb):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        _, viewspace_point_tensor, visibility_filter, radii, objects = render_pkg["render"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"], render_pkg["render_object"]
        ...
        with torch.no_grad():
            ...
            # Log and save
            training_report(iteration, loss, iter_start.elapsed_time(iter_end), loss_obj_3d, use_wandb)
...
def training_report(iteration, loss, elapsed, loss_obj_3d, use_wandb):

    if use_wandb:
        if loss_obj_3d:
            wandb.log({"train_loss_patches/total_loss": loss.item(), "train_loss_patches/loss_obj_3d": loss_obj_3d.item(), "iter_time": elapsed, "iter": iteration})
        else:
            wandb.log({"train_loss_patches/total_loss": loss.item(), "iter_time": elapsed, "iter": iteration})
